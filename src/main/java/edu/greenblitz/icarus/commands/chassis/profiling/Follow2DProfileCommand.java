package edu.greenblitz.icarus.commands.chassis.profiling;
...
public class Follow2DProfileCommand implements IThreadable {
    ...
    public Follow2DProfileCommand(
            ...
            boolean isReverse) {
        this(path, config,
                (RobotMap.Icarus.Chassis.MotionData.PROF.getValue().containsKey(Double.toString(maxPower))) ?
                RobotMap.Icarus.Chassis.MotionData.PROF.getValue().get(Double.toString(maxPower)) : retNullandThrow(), maxPower, isReverse);
    }
    ...
    public Follow2DProfileCommand(List<State> path,
        ...
        follower = new PidFollower2D(linKv, linKa, linKv, linKa,
                ...
                RobotMap.Icarus.Chassis.WHEEL_DIST,
                profile2D);
        follower.setConverter(new CurvatureConverter(RobotMap.Icarus.Chassis.WHEEL_DIST));
    }
